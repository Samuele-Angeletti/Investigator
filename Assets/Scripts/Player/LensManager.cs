...
public class LensManager : MonoBehaviour
{
    ...
    [Tooltip("Raggio massimo entro cui il giocatore pu� rilevare gli indizi (in metri).")]
    [SerializeField] private float _revealRadius = 10f;

    [Tooltip("Soglia di visibilit� minima richiesta dall'indizio per essere evidenziato.")]
    ...
    private void ExecuteInvestigationLoop()
    {
        ...
        for (int i = 0; i < _activeEvidenceModels.Count; i++)
        {
            ...
            if (evidence.EvidenceNode.EvidenceType == EEvidenceType.FOOTSTEPS)
            {
                ...
                {
                    // Ridimensioniamo l'array cache solo se il LineRenderer corrente ha pi� punti
                    ...
                    if (footprintFound)
                    {
                        // Calcolo del guadagno di visibilit� in base al punto pi� vicino trovato
